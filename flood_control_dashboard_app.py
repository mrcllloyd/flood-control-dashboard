...
def load_data() -> pd.DataFrame:
    source = "flood_control_v6.xlsx"  # <--- file in repo root
    ...
    # geo flag
    df["HasGeo"] = (~df.get("Latitude", pd.Series([np.nan]*len(df))).isna()) & \
                   (~df.get("Longitude", pd.Series([np.nan]*len(df))).isna())
    return df

df = load_data()

st.title("Flood Control Projects — Dashboard")
...
sel_years = (year_min, year_max) if (year_min and year_max) else (None, None)

if year_min and year_max:
    sel_years = st.sidebar.slider("Year Range", min_value=year_min, max_value=year_max,
                                  value=(year_min, year_max), step=1)

regions     = ["All"] + (sorted(df["Region"].dropna().unique())     if "Region"     in df.columns else [])
provinces   = ["All"] + (sorted(df["Province"].dropna().unique())   if "Province"   in df.columns else [])
deos        = ["All"] + (sorted(df["DEO"].dropna().unique())        if "DEO"        in df.columns else [])
contractors = ["All"] + (sorted(df["Contractor"].dropna().unique()) if "Contractor" in df.columns else [])
...
sel_cost = st.sidebar.slider("Contract Cost (₱)", min_value=0.0, max_value=max(1000.0, max_cost),
                             value=(0.0, max_cost))
...
def apply_eq(frame, col, val):
    return frame if (col not in frame.columns or val == "All") else frame[frame[col] == val]
...
st.dataframe(
    f[show_cols].sort_values(["Region","Province","YearParsed"], na_position="last"),
...
st.caption("Adjust filters in the left sidebar. The map, charts, and table update instantly. All rows in flood_control_v6.xlsx are used—no sampling.")
